...
2026-08-10 against Discovery revision `20260805`; the scopes row re-verified 2026-09-14 against
revision `20260909`.
...
| Which scopes an operation accepts | No Google source lists them all, and the gaps run both ways: `location.writeonly` left Discovery at revision `20260826` but `create`, `patch` and `batchDelete` still document it; `nutrition.readonly` has never been in Discovery; the three Women's Health `.readonly` scopes arrived in Discovery at revision `20260909` and no reference page lists them yet | The union of Discovery and the per-method reference | [authentication.md](authentication.md#scopes) |
